...
import { PublicTournamentChipPoolWindow } from './PublicTournamentChipPoolWindow';
import { WifiQr } from '@/components/WifiQr/WifiQr';
import { usePublicTournament } from '@/core/states/tournaments/hooks/usePublicTournament';

export interface PublicTournamentDisplayShellProps {
  readonly tournamentId: string;
}

const tvOverrides: CSSProperties & Record<string, string> = {
  '--chip-broadcast-title-size': 'clamp(1.1rem, min(2.5vw, 4vh), 2.5rem)',
  '--chip-broadcast-stat-label-size': 'clamp(0.88rem, min(1.65vw, 2.6vh), 1.4rem)',
  ...
  '--chip-broadcast-subheader-font': 'clamp(0.8rem, min(1.5vw, 2.2vh), 1.15rem)',
...
export const PublicTournamentDisplayShell: FC<PublicTournamentDisplayShellProps> = ({
  tournamentId,
}) => {
  const { data: tournament } = usePublicTournament(tournamentId);
  ...
  return (
    ...
    >
      <PublicTournamentChipPoolWindow
